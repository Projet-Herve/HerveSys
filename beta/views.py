...
article_extractor = SourceFileLoader(
    "", "../Article-Extractor/__init__.py").load_module()
# Import Arduino_Python
arduino = SourceFileLoader(
    "", "../Arduino-Python-1.0/arduino.py").load_module()
...
def cachedelay(date, delay=5):
    time = datetime.datetime.now() - datetime.datetime.strptime(date,
                                                                '%Y-%m-%d %H:%M:%S.%f')
...
def actus_blog():
    actus_json = requests.get(
        'http://ajax.googleapis.com/ajax/services/feed/load?v=1.0&num=10&q=http://herveprojet.hol.es/blog/category/info/feed/')
    try:
...
#############
    ...
    def typefile(i, type_, liste):
        ...
        for y in liste:
            if i.split('.')[-1] == y:
                ...
                if type_file:
                    cloudfiles.append(
                        {'Files': i, 'extention': i.split('.')[-1], 'type': type_file})
            elif i.split('.')[-2:-1] == y:
                #print( i.split('.')[-2:-1])
                type_file = type_
                if type_file:
                    cloudfiles.append(
                        {'Files': i, 'extention': i.split('.')[-2:-1], 'type': type_file})
    ...
    for i in cloudfilesliste:
        ...
        if po is not True:
            cloudfiles.append(
                {"Files": i, "extention": i.split('.')[-1], "type": "inconnu"})
...
###################
    ...
    with open('datas/preferences.json', 'r+') as file:
        ...
        for serveur in preferences['serveur']:
            serveur['lancement'] = serveur['lancement'].format(
                serveur_url=preferences['serveur_url'][:-5])
            serveur['acces'] = serveur['acces'].format(
                serveur_url=preferences['serveur_url'][:-5])
...
#############
    ...
    if a == nbitem:
        ...
        if meteoresult != "Météo indisponible :/":
            meteoresult = {"minimal": meteoresult[
                0], "maximale": meteoresult[1], "tendance": meteoresult[2]}
            donnee.append({"meteo": meteoresult})
        else:
            meteoresult = {"minimal": "Météo indisponible :/",
                           "maximale": "Météo indisponible :/", "tendance": "Météo indisponible :/"}
            donnee.append({"meteo": meteoresult})
    ...
    while script == 'run':
        ...
        try:
            ...
            if etendue == 0:
                ...
                # On est dans le cas ou les deux intervales ne sont pas de meme
                # durée
                if time1 >= 2:
...
def chatbot(request):
    chatbot = ia.chatbot(
        config={'learn': False, 'learn_file': 'datas/learn.py', 'debug': False})
...
def articlereader(request):
    ...
    if request.GET.get("url"):
        article = article_extractor.init(url=request.GET.get(
            "url"), file='../Article-Extractor/website.json')
    ...
    if request.method == 'POST':
        if request.POST.get("modifiquation"):
            with open('static/default/cloud/' + request.POST.get("modifiquation")) as file:
                fileextention = request.POST.get(
                    "modifiquation").split('.')[-1]
        ...
        if request.POST.get('execut'):
            os.system('python3 static/default/cloud/' +
                      str(request.POST.get('file')))
...
def cloud(request):
    ...
    if request.method == 'POST':
        if request.POST.get("delet"):
            os.system('cd static/default/cloud && rm -R ' +
                      request.POST.get("delet"))
            return render(request, 'html/multimedia.html', {'files': cloudfiles, 'menuliste': liste_menu})
...
def cloud_envoyer_fichier(request):
    ...
    if request.GET.get('nom_fichier'):
        fichier_base_64 = fichier_vers_base64(
            'static/default/cloud/' + request.GET.get('nom_fichier'))
...
def reveil_web(request):
    ...
    if request.method == 'POST':
        if request.POST.get("heures_travail") and request.POST.get("minutes_travail") and request.POST.get("heure_preparation") and request.POST.get("minutes_preparation") and request.POST.get("heures_itineraire") and request.POST.get("minutes_itineraire") and request.POST.get("vitesse"):
            # Travail
            minutes_heures_travail = int(heure_vers_minute(
                int(request.POST.get("heures_travail"))))
            minutes_travail = int(request.POST.get("minutes_travail"))
            # Préparation
            minutes_heure_preparation = int(heure_vers_minute(
                int(request.POST.get("heure_preparation"))))
            minutes_preparation = int(request.POST.get("minutes_preparation"))
            # Itinéraire
            minutes_heures_itineraire = int(heure_vers_minute(
                int(request.POST.get("heures_itineraire"))))
    ...
    if request.GET.get('nom_user'):
        with open('datas/preferences.json', 'r+') as file:
            ...
            for i in request.GET:
                ...
                if 'mail' in i:
                    preferences['mail'][
                        i.split('mail')[1]] = request.GET.get(i)
            preferences_json = json.dumps(
                preferences, indent=4, ensure_ascii=False)
            file.seek(0)
...
def action(request):
    if request.GET.get('annee') and request.GET.get('mois') and request.GET.get('jours') and request.GET.get('heure') and request.GET.get('minute') and request.GET.get('action'):
        ...
        cmd_list = ["cd {dossier_script} && python3 manage.py action {id}".format(
            dossier_script=dossier_script, id=id)]
        job = atjob.Atjob()
        job.add(cmd_list)
        job_id = job.create(date, remove=False)
        action.append({'annee': annee, 'mois': mois, 'jours': jours,	'heure': heure, 'minute': minute, 'action': action_,
                       'text': text, 'terminal': 'espeak \'{}\''.format(text), 'job_id': job_id, 'id': id, 'temps': str(date)})
